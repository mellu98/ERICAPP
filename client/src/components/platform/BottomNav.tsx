...
export function BottomNav({ sections, activeSectionId, onSectionChange }: BottomNavProps) {
  return (
    <nav
      className="md:hidden flex items-center border-t border-white/[0.07] bg-[rgba(7,7,17,0.95)] backdrop-blur-xl shrink-0"
      ...
      {sections.map((section) => {
        ...
        return (
          <button
            ...
            className="flex flex-col items-center gap-1 flex-1 py-2.5 transition-all duration-150 active:scale-95"
          >
            <div className="relative">
              <div
                className={cn(
                  'w-11 h-11 rounded-2xl flex items-center justify-center font-bold text-sm border transition-all duration-200',
                  isActive
                    ? toneActiveStyles[section.tone]
                    : 'text-white/30 border-white/[0.07] bg-white/[0.03]',
                )}
              >
                {section.name[0]}
              </div>
              {section.unreadCount ? (
                <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#00d4aa] border border-[#070711]" />
              ) : null}
            </div>
            <span
              className={cn(
                'text-[10px] font-semibold transition-colors',
                isActive ? toneLabelStyles[section.tone] : 'text-white/25',
              )}
            >
              {section.name}
            </span>
